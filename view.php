...
// $stmt = $pdo->prepare('SELECT * FROM tickets WHERE ticket_id = ? ORDER BY created DESC');
...
<!-- original
   ...
   -->
	...
	<h2>Customer Name: <?=htmlspecialchars($ticket['title'], ENT_QUOTES)?> </span></h2>
    <h2>Ticket From: <?=htmlspecialchars($ticket['fl'], ENT_QUOTES)?> </span></h2>
    <h2>To:  <?=htmlspecialchars($ticket['ll'], ENT_QUOTES)?> </h2>
    <h2>Journey Date:  <?=htmlspecialchars($ticket['datee'], ENT_QUOTES)?> </h2>
    <h2>Busname:   <?=htmlspecialchars($ticket['busname'], ENT_QUOTES)?> </h2>
    <h2>SeatNo:  <?=htmlspecialchars($ticket['seatno'], ENT_QUOTES)?> </h2>
    ...
    <div class="btns">
        ...
        <div class="text-center">
          <button onclick="window.print();" class="btn btn-warning" id="print-btn" style="background-color: orange;">Print</button>
        </div>
    </div>
    ...
    <div class="comments">
        ...
        <div class="comment">
            ...
            <p>
                ...
                <?=nl2br(htmlspecialchars($comment['msg'], $comment['fl'], $comment['ll'], ENT_QUOTES))?>
            </p>
        </div>
        <?php endforeach; ?>
